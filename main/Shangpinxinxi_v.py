...
def shangpinxinxi_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __isAdmin__ == "是" and 'shangpinxinxi' != 'forum' :
            if req_dict.get("userid") and 'shangpinxinxi' != 'chat' and 'shangpinxinxi' != 'examrecord':
                del req_dict["userid"]
        else:
            if tablename!="users" and tablename!="jdfnl" and 'shangpinxinxi'[:7]!='discuss' and "userid" in shangpinxinxi.getallcolumn(shangpinxinxi,shangpinxinxi):
                req_dict["userid"] = request.session.get("params").get("id")
...
def shangpinxinxi_query(request):
    '''
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        try:
            query_result = shangpinxinxi.objects.filter(**request.session.get("req_dict")).values()
...
def shangpinxinxi_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        idOrErr= shangpinxinxi.createbyreq(shangpinxinxi,shangpinxinxi, req_dict)
...
def shangpinxinxi_autoSort2(request):
    if request.method in ["POST", "GET"]:
        ...
        user_ratings={}
        try:
            cursor.execute("select * from storeup where type = 1 and tablename = 'shangpinxinxi' order by addtime desc")
        ...
        if where:
            sql = f'''SELECT * FROM (SELECT * FROM shangpinxinxi WHERE {where}) AS table1 WHERE id IN ('{"','".join(sorted_recommended_goods)}') union all SELECT * FROM (SELECT * FROM shangpinxinxi WHERE {where}) AS table1 WHERE id NOT IN ('{"','".join(sorted_recommended_goods)}')'''
        else:
            sql = f'''select * from shangpinxinxi where id in ('{"','".join(sorted_recommended_goods)}') union all select * from shangpinxinxi where id not in('{"','".join(sorted_recommended_goods)}')'''
        cursor.execute(sql)
        desc = cursor.description
        data_dict = [dict(zip([col[0] for col in desc], row)) for row in cursor.fetchall()]
