...
def schemaName_spider(request, tableName):
    if request.method in ["POST", "GET"]:
        ...
        # Linux
        cmd = "cd /yykj/python/9999/spider${spiderSchemaName} && scrapy crawl "+tableName+"Spider -a databaseName=django85gv12pu"
        # Windows
        # cmd = "cd C:\\test1\\spider && scrapy crawl " + tableName + "Spider -a databaseName=django85gv12pu"
        os.system(cmd)
...
def get_access_token():
    ...
    params = {"grant_type": "client_credentials", "client_id": '8tOAUqbKuKnr71TPMyFEZekn', "client_secret": 'iqRDT7HOJgyZcVQy2MtWODY27trwDlrt'}
    return str(requests.post(url, params=params).json().get("access_token"))

def schemaName_askai(request):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "data": ""}
        req_dict = request.session.get("req_dict")
