...
def systemintro_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __isAdmin__ == "是" and 'systemintro' != 'forum' :
            if req_dict.get("userid") and 'systemintro' != 'chat' and 'systemintro' != 'examrecord':
                del req_dict["userid"]
        else:
            if tablename!="users" and tablename!="jdfnl" and 'systemintro'[:7]!='discuss' and "userid" in systemintro.getallcolumn(systemintro,systemintro):
                req_dict["userid"] = request.session.get("params").get("id")
...
def systemintro_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        idOrErr= systemintro.createbyreq(systemintro,systemintro, req_dict)
