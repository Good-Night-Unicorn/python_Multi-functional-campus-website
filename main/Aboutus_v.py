...
def aboutus_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __isAdmin__ == "是" and 'aboutus' != 'forum' :
            if req_dict.get("userid") and 'aboutus' != 'chat' and 'aboutus' != 'examrecord':
                del req_dict["userid"]
        else:
            if tablename!="users" and tablename!="jdfnl" and 'aboutus'[:7]!='discuss' and "userid" in aboutus.getallcolumn(aboutus,aboutus):
                req_dict["userid"] = request.session.get("params").get("id")
...
def aboutus_query(request):
    '''
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        try:
            query_result = aboutus.objects.filter(**request.session.get("req_dict")).values()
...
def aboutus_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        idOrErr= aboutus.createbyreq(aboutus,aboutus, req_dict)
