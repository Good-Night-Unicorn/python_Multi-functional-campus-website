	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/dingdanxinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">发货时间</div>
						<div class="text "  >{{detail.fahuoshijian}}</div>
					</div>
					<div class="item">
						<div class="lable">发货详情</div>
						<div class="text "  >{{detail.fahuoxiangqing}}</div>
					</div>
					<div class="item">
						<div class="lable">跑腿账号</div>
						<div class="text "  >{{detail.paotuizhanghao}}</div>
					</div>
					<div class="item">
						<div class="lable">跑腿姓名</div>
						<div class="text "  >{{detail.paotuixingming}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">发货状态</div>
						<div class="text "  >{{detail.fahuozhuangtai}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('dingdanxinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('dingdanxinxi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('dingdanxinxi','配送')" 
							@click="onAcross('dingdanpeisong','','','fahuozhuangtai','已发货','已发货,未发货'.split(',')[0])" type="warning">
							配送
						</el-button>
					</div>
				</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/dingdanxinxi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此订单信息？') .then(_ => {
					this.$http.post('dingdanxinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
