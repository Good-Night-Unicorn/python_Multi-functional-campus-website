<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="配送地址" prop="shouhuodizhi">
				<el-input v-model="ruleForm.shouhuodizhi" 
					placeholder="配送地址" clearable :disabled=" false  ||ro.shouhuodizhi"></el-input>
			</el-form-item>
			<el-form-item class="add-item"  label="配送状态" prop="peisongzhuangtai">
				<el-select v-model="ruleForm.peisongzhuangtai" placeholder="请选择配送状态" :disabled=" false  ||ro.peisongzhuangtai" >
					<el-option
			...
			<el-form-item class="add-item" label="配送时间" prop="peisongshijian">
				<el-date-picker
					:disabled=" false  ||ro.peisongshijian"
			...
			<el-form-item class="add-item" label="跑腿账号" prop="paotuizhanghao">
				<el-input v-model="ruleForm.paotuizhanghao" 
					placeholder="跑腿账号" clearable :disabled=" false  ||ro.paotuizhanghao"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		created() {
			...
			this.ruleForm.peisongshijian = this.getCurDate()
		},
		methods: {
			getMakeZero(s) {
				return s < 10 ? '0' + s : s;
			...
			init(type) {
				...
				this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						...
						if((json.paotuizhanghao!=''&&json.paotuizhanghao) || json.paotuizhanghao==0){
							this.ruleForm.paotuizhanghao = json.paotuizhanghao;
							this.ro.paotuizhanghao = true;
						}
						if((json.paotuixingming!=''&&json.paotuixingming) || json.paotuixingming==0){
							this.ruleForm.paotuixingming = json.paotuixingming;
							this.ro.paotuixingming = true;
						}
					}
				});
				this.peisongzhuangtaiOptions = "已送达,配送中".split(',')
			...
			// 多级联动参数
			info() {
				this.$http.get(`dingdanpeisong/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`dingdanpeisong/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
