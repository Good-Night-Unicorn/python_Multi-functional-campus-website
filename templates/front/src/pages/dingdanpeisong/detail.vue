	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/dingdanpeisong?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">配送地址</div>
						<div class="text "  >{{detail.shouhuodizhi}}</div>
					</div>
					<div class="item">
						<div class="lable">配送状态</div>
						<div class="text "  >{{detail.peisongzhuangtai}}</div>
					</div>
					<div class="item">
						<div class="lable">配送时间</div>
						<div class="text "  >{{detail.peisongshijian}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('dingdanpeisong','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('dingdanpeisong','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/dingdanpeisong', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/dingdanpeisongAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此订单配送？') .then(_ => {
					this.$http.post('dingdanpeisong/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
