	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/shangpingoumai?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">购买价格</div>
						<div class="text "  >{{detail.shangpinjiage}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.shouhuodizhi}}</div>
					</div>
					<div class="item">
						<div class="lable">购买账号</div>
						<div class="text "  >{{detail.goumaizhanghao}}</div>
					</div>
					<div class="item">
						<div class="lable">购买姓名</div>
						<div class="text "  >{{detail.goumaixingming}}</div>
					</div>
					<div class="item">
						<div class="lable">状态</div>
						<div class="text "  >{{detail.zhuangtai}}</div>
					</div>
					<div class="item" v-if="btnAuth('shangpingoumai','支付')">
						<div class="lable">是否支付</div>
						<div class="text">{{detail.ispay=='已支付'?'已支付':'未支付'}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('shangpingoumai','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('shangpingoumai','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('shangpingoumai','发货')" 
							@click="onAcross('dingdanxinxi','','','','')" type="warning">
							发货
						</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('shangpingoumai','提现')" 
							@click="onAcross('tixianxinxi','','','zhuangtai','已提现','已提现,未提现'.split(',')[0])" type="warning">
							提现
						</el-button>
						<el-button class="payBtn" v-if="(detail.ispay=='未支付'||!detail.ispay)&&btnAuth('shangpingoumai','支付')" type="warning" size="small" @click="payClick">支付</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="购买详情" name="first">
					<div class="ql-snow ql-editor" v-html="detail.goumaixiangqing"></div>
				</el-tab-pane>
			</el-tabs>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`shangpingoumai`);
			...
			payClick(){
				localStorage.setItem('paytable','shangpingoumai')
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/shangpingoumai', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/shangpingoumaiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此商品购买？') .then(_ => {
					this.$http.post('shangpingoumai/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
