<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="购买价格" prop="shangpinjiage">
				<el-input-number v-model="ruleForm.shangpinjiage" placeholder="购买价格" :disabled=" false ||ro.shangpinjiage"></el-input-number>
			</el-form-item>
			...
			<el-form-item class="add-item" label="购买详情" prop="goumaixiangqing">
				<editor 
					v-model="ruleForm.goumaixiangqing" 
					class="editor" 
					action="file/upload">
				</editor>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='goumaixiangqing'){
							this.ruleForm.goumaixiangqing = obj[o];
							this.ro.goumaixiangqing = true;
							continue;
						}
						if(o=='yonghuzhanghao'){
				...
				this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						...
						if((json.yonghuzhanghao!=''&&json.yonghuzhanghao) || json.yonghuzhanghao==0){
							this.ruleForm.goumaizhanghao = json.yonghuzhanghao;
							this.ro.goumaizhanghao = true;
						}
						if((json.yonghuxingming!=''&&json.yonghuxingming) || json.yonghuxingming==0){
							this.ruleForm.goumaixingming = json.yonghuxingming;
							this.ro.goumaixingming = true;
						}
					}
				});
				this.zhuangtaiOptions = "已提现,未提现".split(',')
			...
			// 多级联动参数
			info() {
				this.$http.get(`shangpingoumai/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`shangpingoumai/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
									duration: 1500,
									onClose: () => {
										if(this.isBackAuth('shangpingoumai','支付')&&this.type=='cross'){
											this.$confirm('是否跳转支付？') .then(_ => {
												let jumpParams = {
													id: res.data.data,
													centerType: 1
												}
												this.$router.push({path: '/index/shangpingoumaiDetail', query: jumpParams});
											}).catch(_ => {
												this.$router.go(-1);
											});
										}else {
