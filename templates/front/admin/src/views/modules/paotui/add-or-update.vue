		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="跑腿账号" prop="paotuizhanghao" >
					<el-input v-model="ruleForm.paotuizhanghao" placeholder="跑腿账号" clearable  :readonly="ro.paotuizhanghao"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="跑腿账号" prop="paotuizhanghao" >
	...
	export default {
		data() {
			...
			return {
				...
				rules: {
					paotuizhanghao: [
						{ required: true, message: '跑腿账号不能为空', trigger: 'blur' },
					],
					mima: [
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `paotui/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.paotuiCrossAddOrUpdateFlag = false;
											this.parent.search();
