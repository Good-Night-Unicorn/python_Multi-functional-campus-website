<template>
	<div>
		<div class="register-container">
			...
			<el-form v-if="pageFlag=='security1'" class="rgs-form animate__animated animate__flipInY" :model="forgetForm">
				<div class="rgs-form2">
					<div class="title">基于DJANGO框架的多功能校园网站的设计与实现找回密码</div>
			...
			<el-form v-if="pageFlag=='security2'" class="rgs-form animate__animated animate__flipInY" :model="ruleForm">
				<div class="rgs-form2">
					<div class="title">基于DJANGO框架的多功能校园网站的设计与实现密码重置</div>
					<el-form-item class="list-item">
			...
			<el-form v-if="pageFlag=='security3'" class="rgs-form animate__animated animate__flipInY" :model="ruleForm">
				<div class="rgs-form2">
					<div class="title">基于DJANGO框架的多功能校园网站的设计与实现密码重置</div>
					<el-form-item class="list-item">
			...
			<el-form v-if="pageFlag=='register'" ref="ruleForm" class="rgs-form animate__animated animate__flipInY" :model="ruleForm" :rules="rules">
				<div class="rgs-form2">
					<div class="title">基于DJANGO框架的多功能校园网站的设计与实现</div>
					...
					<el-form-item class="list-item" v-if="tableName=='paotui'">
						<div class="lable" :class="changeRules('paotuizhanghao')?'required':''">跑腿账号：</div>
						<el-input  v-model="ruleForm.paotuizhanghao"  autocomplete="off" placeholder="跑腿账号"  type="text"  />
					</el-form-item>
					<el-form-item class="list-item" v-if="tableName=='paotui'">
						<div class="lable" :class="changeRules('mima')?'required':''">密码：</div>
						<el-input  v-model="ruleForm.mima"  autocomplete="off" placeholder="密码"  type="password"  />
					...
					<el-form-item class="list-item" v-if="tableName=='paotui'">
						<div class="lable" :class="changeRules('paotuixingming')?'required':''">跑腿姓名：</div>
						<el-input  v-model="ruleForm.paotuixingming"  autocomplete="off" placeholder="跑腿姓名"  type="text"  />
					</el-form-item>
					<el-form-item class="list-item" v-if="tableName=='paotui'">
						<div class="lable" :class="changeRules('touxiang')?'required':''">头像：</div>
						<file-upload
...
export default {
	...
	mounted(){
		...
		if(this.$route.query.pageFlag=='register'){
			...
			if ('paotui' == this.tableName) {
				this.rules.paotuizhanghao = [{ required: true, message: '请输入跑腿账号', trigger: 'blur' }]
			}
			if ('paotui' == this.tableName) {
				this.rules.mima = [{ required: true, message: '请输入密码', trigger: 'blur' }]
			}
			if ('paotui' == this.tableName) {
				this.rules.paotuixingming = [{ required: true, message: '请输入跑腿姓名', trigger: 'blur' }]
			}
	...
	methods: {
		...
		login() {
			...
			if((!this.ruleForm.paotuizhanghao) && `paotui` == this.tableName){
				this.$message.error(`跑腿账号不能为空`);
				return
			}
			if((!this.ruleForm.mima) && `paotui` == this.tableName){
				this.$message.error(`密码不能为空`);
				return
			}
			if((this.ruleForm.mima!=this.ruleForm.mima2) && `paotui` == this.tableName){
				this.$message.error(`两次密码输入不一致`);
				return
			}
			if((!this.ruleForm.paotuixingming) && `paotui` == this.tableName){
			...
			if(`paotui` == this.tableName && this.ruleForm.lianxidianhua &&(!this.$validate.isMobile(this.ruleForm.lianxidianhua))){
				this.$message.error(`联系电话应输入手机格式`);
				return
			}
			if(`paotui` == this.tableName && this.ruleForm.pquestion==''){
				this.$message.error(`密保问题不能为空`);
				return
			}
			if(`paotui` == this.tableName && this.ruleForm.panswer==''){
				this.$message.error(`密保答案不能为空`);
				return
			}
			this.$http({
